...
class WindowVAE(nn.Module):
    """
    Local VAE applied independently at each time step.
    """
    def __init__(
        self,
        in_dim: int,
...
class UtteranceDisentangler(nn.Module):
    """
    Shared encoder is reused for both modalities.
    Private encoders remain modality-specific.
    """
    def __init__(
        self,
        pooled_dim: int,
    ...
    def forward_one(self, pooled_x: torch.Tensor, modality: str) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "recon": recon,
            "loss_recon": loss_recon,
            "loss_kl_shared": loss_kl_shared,
            "loss_kl_private": loss_kl_private,
...
class TokenReasoner(nn.Module):
    """
    Task-agnostic token reasoning block that preserves DIVINE's core idea.
    """
    def __init__(
        self,
        token_dim: int,
    ...
    def forward(self, fused_h: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "Hout": h_out,
            "h_fused_out": h_out[:, -1, :],
            "loss_token": F.mse_loss(h_out[:, :-1, :], token_bank, reduction="mean"),
        }
