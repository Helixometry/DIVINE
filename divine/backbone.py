...
from .modules import MLP, TemporalRefiner, TokenReasoner, UtteranceDisentangler, WindowVAE

@dataclass
class DIVINEBackboneConfig:
    mod1_in_dim: int
    mod2_in_dim: int
    refine_dim: int = 128
...
class DIVINEBackbone(nn.Module):
    def __init__(self, cfg: DIVINEBackboneConfig) -> None:
        super().__init__()
        self.cfg = cfg
        ...
        self.utterance = UtteranceDisentangler(cfg.window_latent_dim, cfg.shared_dim, cfg.private_dim,
                                               beta_shared=cfg.beta_shared, beta_private=cfg.beta_private, dropout=cfg.dropout)
        ...
        self.fused_to_token_dim = nn.Identity() if cfg.shared_dim == cfg.token_dim else nn.Linear(cfg.shared_dim, cfg.token_dim)
        self.token_reasoner = TokenReasoner(cfg.token_dim, cfg.num_tokens, hidden_dim=max(128, cfg.token_dim), dropout=cfg.dropout)

    def _encode_modality(self, x: torch.Tensor, refiner: nn.Module, window_vae: nn.Module, modality_name: str) -> Dict[str, torch.Tensor]:
        refined = refiner(x)
        if self.cfg.use_local_vae:
            ...
            local = {"z_seq": refined, "recon_seq": refined, "mu": refined, "logvar": refined, "loss_recon": zero, "loss_kl": zero, "loss_total": zero}
        utter = self.utterance.forward_one(pooled, modality_name)
        return {"input": x, "refined": refined, "pooled": pooled, **{f"local_{k}": v for k, v in local.items()}, **{f"utt_{k}": v for k, v in utter.items()}}
    ...
    def forward(self, mod1_x: Optional[torch.Tensor] = None, mod2_x: Optional[torch.Tensor] = None,
                mod1_present: Optional[torch.Tensor] = None, mod2_present: Optional[torch.Tensor] = None) -> Dict[str, Any]:
